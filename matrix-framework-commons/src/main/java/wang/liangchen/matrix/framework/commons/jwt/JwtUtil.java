...
public enum JwtUtil {
    ...
    public String generateKey(int length) {
        //32 48 64
        ValidationUtil.INSTANCE.isTrue(MatrixExceptionLevel.WARN, length == 32 || length == 48 || length == 64, "length must be equal to 32,48,64");
    ...
    public String sign(JWSAlgorithm algorithm, String secretKey, JWTClaimsSet claimsSet) {
        ValidationUtil.INSTANCE.notNull(MatrixExceptionLevel.WARN, algorithm, "algorithm must not be null");
        ValidationUtil.INSTANCE.notBlank(MatrixExceptionLevel.WARN, secretKey, "secretKey must not be blank");
    ...
    public JWTClaimsSet verify(String secretKey, String jwtString) {
        ValidationUtil.INSTANCE.notBlank(MatrixExceptionLevel.WARN, secretKey, "secretKey must not be blank");
        ValidationUtil.INSTANCE.notBlank(MatrixExceptionLevel.WARN, jwtString, "jwtString must not be blank");
        byte[] sharedSecret = Base64Util.INSTANCE.decode(secretKey);
        int length = sharedSecret.length;
        ValidationUtil.INSTANCE.isTrue(MatrixExceptionLevel.WARN, length == 32 || length == 48 || length == 64, "length must be equal to 32,48,64");
        try {
    ...
    public String rsaSign(JWSAlgorithm algorithm, String privateKeyString, JWTClaimsSet claimsSet) {
        ValidationUtil.INSTANCE.notNull(MatrixExceptionLevel.WARN, algorithm, "algorithm must not be null");
        ValidationUtil.INSTANCE.isTrue(MatrixExceptionLevel.WARN,algorithm.getName().startsWith("RS"),"algorithm must be RSA family");
    ...
    public JWTClaimsSet rsaVerify(String publicKeyString, String jwtString) {
        ValidationUtil.INSTANCE.notBlank(MatrixExceptionLevel.WARN, publicKeyString, "publicKeyString must not be blank");
        ValidationUtil.INSTANCE.notBlank(MatrixExceptionLevel.WARN, jwtString, "jwtString must not be blank");
