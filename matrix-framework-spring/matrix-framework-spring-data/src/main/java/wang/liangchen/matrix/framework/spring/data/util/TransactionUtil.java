package wang.liangchen.matrix.framework.spring.data.util;
...
public enum TransactionUtil {
     ...
     */
    INSTANCE;
    private final PlatformTransactionManager transactionManager = BeanContext.INSTANCE.getBean(PlatformTransactionManager.class);
    ...
    static class AfterCommitExecutor implements TransactionSynchronization, Executor {
        ...
        private static final Executor executor = ThreadUtil.INSTANCE.getUnboundedExecutor();

        @Override
        public void execute(Runnable runnable) {
            logger.debug("Submitting new runnable {} to run after commit", runnable);
            // 检查当前线程事务是否是活动的事务,如果不是则事务已提交,立即执行
            if (!TransactionSynchronizationManager.isSynchronizationActive()) {
                logger.debug("Transaction synchronization is not active. Executing right now runnable");
