...
public enum IOUtil {
    ...
    public String read(InputStream inputStream, String encoding, int bufferSize) {
        AssertUtil.INSTANCE.notNull(inputStream, "inputStream can not be null");
        if (bufferSize <= 0) {
    ...
    public void io(InputStream inputStream, OutputStream outputStream, int bufferSize) {
        AssertUtil.INSTANCE.notNull(inputStream, "inputStream can not be null");
        AssertUtil.INSTANCE.notNull(outputStream, "outputStream can not be null");
        if (bufferSize <= 0) {
            bufferSize = DEFAULT_BUFFER_SIZE;
        }
        byte[] buffer = new byte[bufferSize];
    ...
    public void io(Reader reader, Writer writer, int bufferSize) {
        AssertUtil.INSTANCE.notNull(reader, "reader can not be null");
        AssertUtil.INSTANCE.notNull(writer, "writer can not be null");
        if (bufferSize <= 0) {
            bufferSize = DEFAULT_BUFFER_SIZE;
        }
