...
abstract class AbstractObjectCriteria<E extends RootEntity> extends AbstractClassCriteria<E> {
    ...
    @Override
    protected AbstractObjectCriteria<E> _equalsIgnoreCase(EntityGetter<E> fieldGetter, String sqlValue) {
        addCriteriaMeta(Operator.EQUALS, fieldGetter, true);
        return this;
    }
    ...
    @Override
    protected AbstractObjectCriteria<E> _notEqualsIgnoreCase(EntityGetter<E> fieldGetter, String sqlValue) {
        addCriteriaMeta(Operator.NOTEQUALS, fieldGetter, true);
        return this;
    }
    ...
    private void addCriteriaMeta(Operator operator, EntityGetter<E> fieldGetter) {
        addCriteriaMeta(operator, fieldGetter, false);
    }

    private void addCriteriaMeta(Operator operator, EntityGetter<E> fieldGetter, boolean ignoreCase) {
        ValidationUtil.INSTANCE.notNull(entity, "Unsupported call,entity must not be null");
        ...
        FieldMeta fieldMeta = getFieldMetas().get(fieldName);
        if (ignoreCase && fieldValue instanceof String) {
            getComposedCriteriaResolver().add(SingleCriteriaResolver.newInstanceIgnoreCase(fieldMeta.getColumnName(), operator, String.valueOf(fieldValue)));
            return;
        }
        getComposedCriteriaResolver().add(SingleCriteriaResolver.newInstance(fieldMeta.getColumnName(), operator, fieldValue));
    }
