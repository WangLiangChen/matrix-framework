            ...
            for (CriteriaMeta<E> criteriaMeta : CRITERIAMETAS) {
                ...
                for (Object sqlValue : sqlValues) {
                    if (sqlValue instanceof EntityGetter) {
                        placeholders.add(columnName((EntityGetter<E>) sqlValue, columnMetas));
                        continue;
                    }
                ...
                switch (operator) {
                    ...
                    case CONTAINS:
                    case NOTCONTAINS:
                        sqlBuilder.append(columnName).append(operator.getOperator()).append("'%").append(sqlValues[0]).append("%'");
                        break;
                    case STARTWITH:
                    case NOTSTARTWITH:
                        sqlBuilder.append(columnName).append(operator.getOperator()).append("'").append(sqlValues[0]).append("%'");
