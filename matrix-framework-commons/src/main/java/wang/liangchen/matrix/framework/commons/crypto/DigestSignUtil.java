package wang.liangchen.matrix.framework.commons.crypto;

import wang.liangchen.matrix.framework.commons.codec.Base64Util;
import wang.liangchen.matrix.framework.commons.collection.CollectionUtil;
import wang.liangchen.matrix.framework.commons.crypto.enums.DigestAlgorithm;
import wang.liangchen.matrix.framework.commons.crypto.enums.HmacAlgorithm;
import wang.liangchen.matrix.framework.commons.crypto.enums.SignatureAlgorithm;
...
public enum DigestSignUtil {
    ...
    public String hmac(HmacAlgorithm aligorithm, String secretKeyString, String dataString) {
        ValidationUtil.INSTANCE.notNullAndBlank(secretKeyString, "secretKeyString must not be blank");
        ValidationUtil.INSTANCE.notNullAndBlank(dataString, "dataString must not be blank");
    ...
    public String hmac(HmacAlgorithm aligorithm, byte[] secretKeyBytes, byte[] dataBytes) {
        ValidationUtil.INSTANCE.notNullAndEmpty(secretKeyBytes, "secretKeyBytes must not be empty");
        ValidationUtil.INSTANCE.notNullAndEmpty(dataBytes, "dataBytes must not be empty");
        try {
    ...
    public String digest(DigestAlgorithm algorithm, String dataString) {
        ValidationUtil.INSTANCE.notNullAndBlank(dataString, "data must not be blank");
        byte[] dataBytes = dataString.getBytes(StandardCharsets.UTF_8);
    ...
    public boolean verify(SignatureAlgorithm algorithm, String publicKeyString, String signatureString, String data) {
        ...
        ValidationUtil.INSTANCE.notNullAndBlank(data, "data must not be blank");
        ValidationUtil.INSTANCE.notNullAndBlank(signatureString, "signatureString must not be blank");
    ...
    public int hashIndex(Object object, int indexScope) {
        ...
        ValidationUtil.INSTANCE.isTrue(number == 0, "indexScope must be a power of 2");
    ...
    public String dictionarySort(Map<String, String> source, String... excludeKeys) {
        ...
        return source.entrySet().stream()
                .filter(e -> !excludeList.contains(e.getKey()) || StringUtil.INSTANCE.isNotNullAndEmpty(e.getValue()))
