 ...
 * 只提供事件唯一标识(uuidEventId)与事件发生时间(occurredOn)两项基础属性，业务数据由具体事件以消费方必要数据为限自行携带；
 ...
 * 注入构造(uuidEventId, occurredOn)用于测试、reconstitute与历史事件回放，保证事件的确定性与可复现。
 *
...
public abstract class AbstractDomainEvent implements IDomainEvent {
    ...
    public String toString() {
        return getClass().getSimpleName() + "(uuidEventId=" + eventId + ", occurredOn=" + occurredOn + ")";
    }
}
