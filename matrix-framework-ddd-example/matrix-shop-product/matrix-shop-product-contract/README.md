...
- 任何业务模块：`domain`、`application`、`infrastructure`、`interfaces`、`client`、`bootstrap`
- 任何技术实现框架（Spring、JPA/Hibernate 等）

## 角色

消息契约，本上下文的发布语言：声明客户端与应用服务之间、以及本上下文与下游限界上下文之间交互的消息类型（`message.request` 请求、`message.response` 结果/视图）。被 `interfaces`、`application`、`client` 引用，自身不依赖任何业务模块。
