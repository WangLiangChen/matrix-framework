...
- `*RepositoryAdapter`（`@Adapter(PortType.Repository)`）：实现 `*RepositoryPort`（含统一语言命名的查询方法），完成聚合与持久化对象（Po）之间的翻译；重建聚合委托领域工厂 `reconstitute`；查询侧从 Po 重建聚合根返回
...
- 查询侧不建立独立读模型存储，统一语言命名的查询方法由同一仓储适配器从 Po 重建聚合根返回
