package liangchen.wang.matrix.framework.web.utils;


import liangchen.wang.matrix.framework.commons.exception.MatrixErrorException;
import liangchen.wang.matrix.framework.web.response.DeferredResultResponse;
...
public enum DeferredResultUtil {
    ...
    public void putDeferredResult(String key, DeferredResult<DeferredResultResponse> deferredResult) {
        ...
        Optional.ofNullable(map).filter(e -> !e.containsKey(key)).orElseThrow(() -> new MatrixErrorException("DeferredResult is existed,key:" + key));
